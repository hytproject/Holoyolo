...
gsigs = 'https://github.com/Holoyolo-project/gitian.sigs.git'
...
def setup():
    ...
    os.chdir('inputs')
    if not os.path.isdir('Holoyolo'):
        subprocess.check_call(['git', 'clone', args.url, 'Holoyolo'])
    os.chdir('..')
...
def rebuild():
    ...
    for i in args.os:
        ...
        infile = 'inputs/Holoyolo/contrib/gitian/gitian-' + tag_name + '.yml'
        subprocess.check_call(['bin/gbuild', '-j', args.jobs, '-m', args.memory, '--commit', 'Holoyolo='+args.commit, '--url', 'Holoyolo='+args.url, infile])
        ...
        subprocess.check_call('mv build/out/Holoyolo-*.' + suffix + ' ../out/'+args.version, shell=True)
...
def build():
    ...
    subprocess.check_call(['make', '-C', 'inputs/Holoyolo/contrib/depends', 'download', 'SOURCES_PATH=' + os.getcwd() + '/cache/common'])
...
def verify():
    ...
    for i, v in platforms:
        ...
        subprocess.check_call(['bin/gverify', '-v', '-d', '../sigs/', '-r', args.version+'-'+v[1], 'inputs/Holoyolo/contrib/gitian/gitian-'+v[1]+'.yml'])
    os.chdir(workdir)
...
def main():
    ...
    parser.add_argument('-u', '--url', dest='url', default='https://github.com/Holoyolo-project/Holoyolo', help='Specify the URL of the repository. Default is %(default)s')
    ...
    os.makedirs('builder/inputs/Holoyolo', exist_ok=True)
    os.chdir('builder/inputs/Holoyolo')
    if args.pull:
