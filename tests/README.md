...
Core tests take longer than any other Holoyolo tests, due to the high amount of computational work involved in validating core components.
...
To run only Holoyolo's core tests (after building):
...
To run only Holoyolo's crypto tests (after building):
...
```bash
Holoyolod --regtest --offline --fixed-difficulty 1
```
...
Open the wallet file with `Holoyolo-wallet-rpc` with RPC port 18083. Finally, start tests by invoking ./blockchain.py or ./speed.py
...
To run only Holoyolo's hash tests (after building):
...
To run only Holoyolo's performance tests (after building):
...
To run only Holoyolo's unit tests (after building):
